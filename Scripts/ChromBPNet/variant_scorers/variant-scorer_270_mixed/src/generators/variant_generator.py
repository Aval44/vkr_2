...
class VariantGenerator(Sequence):
    ...
    def __get_allele_seq__(self, chrom, pos_, allele1, allele2, seed=-1):
        ...
        if len(allele1) == len(allele2):
            flank = str(self.genome[chrom][pos - 270 +  right_flank_window + 1 - 30 : pos + right_flank_window + 1 + 30])
            if self.shuf:
                assert seed != -1
                flank = dinuc_shuffle(flank, rng=np.random.RandomState(seed))

            allele1_seq = flank[:300 - right_flank_window -1] + allele1 + flank[300 - right_flank_window:]
            allele2_seq = flank[:300 - right_flank_window -1] + allele2 + flank[300 - right_flank_window:]
            
            allele1_seq= 892*'N' + allele1_seq + 892*'N'
            allele2_seq= 892*'N' + allele2_seq + 892*'N'
