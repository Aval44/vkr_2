...
class VariantGenerator(Sequence):
    ...
    def __get_allele_seq__(self, chrom, pos_, allele1, allele2, seed=-1):
        ...
        left_N= max(0, self.flank_size - pos)*'N'
        right_N= max(0, self.flank_size + pos - len(self.genome[chrom]))*'N'
        ...
        if len(allele1) == len(allele2):
            if (len(left_N) == 0 ) and (len(right_N) == 0 ):
                ...
                flank  = left_N + str(self.genome[chrom][:pos+self.flank_size])
            elif (len(right_N) != 0 ): 
                flank  = str(self.genome[chrom][pos-self.flank_size:]) + right_N
        ...
        if len(allele1_seq) != self.flank_size * 2:
            print(len(allele1_seq), len(allele2_seq), self.flank_size * 2, pos, strand, len(left_N), len(right_N), chrom, allele1_seq)
